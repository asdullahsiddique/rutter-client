...
describe("Order APIs", () => {
  ...
  it("consumes fetch by id API", async () => {
    ...
    const scope = nock(defaults.API_ENDPOINT)
      .get("/orders/1234?access_token=123&order_id=1234")
      .reply(200, {});
    await client.order.fetchById(options);
    expect(scope.isDone()).toBeTruthy();
  });

  it("consumes fetch by id API and expands transactions", async () => {
    const options = {
      access_token: "123",
      order_id: "1234",
      expand: ["transactions"],
    ...
    const scope = nock(defaults.API_ENDPOINT)
      .get("/orders/1234?access_token=123&expand=transactions&order_id=1234")
      .reply(200, {});
    await client.order.fetchById(options);
    expect(scope.isDone()).toBeTruthy();
  });
